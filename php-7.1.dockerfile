...
RUN curl -fsSL https://github.com/unicode-org/icu/releases/download/release-57-2/icu4c-57_2-src.tgz -o icu4c-57_2-src.tgz && \
    tar -xzf icu4c-57_2-src.tgz && \
    cd icu/source && \
    ./configure --prefix=/usr/local && make && make install &&\
    cd ../../ && rm -rf icu icu4c-57_2-src.tgz
...
RUN curl -fsSL https://downloads.ioncube.com/loader_downloads/ioncube_loaders_lin_x86-64.tar.gz -o ioncube_loaders.tar.gz && \
    ...
    cp ioncube/ioncube_loader_lin_7.1.so /usr/local/php/lib/php/extensions/$EXT_DIR/ioncube_loader_lin_7.1.so && \
    echo "zend_extension=ioncube_loader_lin_7.1.so" >> /usr/local/php/php.ini && \
